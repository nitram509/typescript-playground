...
/// <reference path="typings/halson/halson.d.ts" />
...
app.get('/', (req, res) => {
  ...
  if (accepts === 'html') {
    ...
    res.set('Content-Type', 'application/hal+json');
    fs.readFile(STATIC_FOLDER + '/index.hal.json', {autoClose : true, flag : 'r', encoding : 'utf-8'}, function (err, data) {
      if (err) throw err;
